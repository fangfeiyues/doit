...
1. 读未提交：可以读取到其他事务未提交的内容。可能导致脏读(读到未提交的数据)
2. 读提交：只能读取到其他事务提交的内容。可能导致不可重复读(两次查询之间有了一次更新)。一般数据库的默认方式但非mysql
3. 可重复读：同一个事务在多实例并发读取数据时可以看到同样的数据。可能导致幻读
4. 串型化：强制事务排序，使之不能相互冲突解决幻读

> 1. "读未提交" 隔离级别下直接返回最新值
> 2. "读提交" 这个视图是在每个SQL语句开始执行的时候创建
> 3. "可重复度" 这个视图是在事务启动的时候创建 整个事务在期间都用这个视图
> 4. "串行化" 直接加锁的方式来避免并行访问
```
